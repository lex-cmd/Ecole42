...
public class MlxMain {
      ...
      var loopHookTimer:CFRunLoopimer?
...
///         mlx.doCallLoopHook()
    ...
    func createTCallback() -> CFRunLoopimerCallBack
    {
        return { (cfRunloopimer, info) -> Void in
	    let mlx:MlxMain = _mlx_bridge(ptr:info!)
            mlx.doCallLoopHook()
        }
    }
    ...
    public func addLoopHook(_ f:UnsafeMutableRawPointer?, _ p:UnsafeMutableRawPointer)
      {
        var tcontext = CFRunLoopimerContext(version:0, info:_mlx_bridge(obj:self), retain:nil, release:nil, copyDescription:nil)
	if (loopHook != nil)
	{
		CFRunLoopimerInvalidate(loopHookTimer)
	}
	...
	if (loopHook != nil)
	{
	   loopHookTimer = CFRunLoopimerCreate(kCFAllocatorDefault, 0.0, 0.0001, 0, 0, createTCallback(), &tcontext)
